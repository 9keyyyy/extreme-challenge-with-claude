...
### Repository 추상화 — Protocol 도입 타이밍

Phase 1에서는 Service가 구체 클래스(PostRepository)에 직접 의존했음. 느슨한 결합이 아니었음. 카운터를 Redis로 교체하는 이 Phase가 **Protocol(인터페이스)을 도입하기 가장 좋은 타이밍**임.

**왜 지금인가:** "카운터 Repository를 DB 기반에서 Redis 기반으로 교체"하는 실제 필요가 생겼기 때문. 추상화는 필요할 때 도입해야 의미가 있음. Phase 1에서 미리 만들어두면 "왜 필요한지"를 모르고 형식만 따라하게 됨.

```python
# Protocol 정의 (인터페이스 역할)
...
# DB 구현체 (Phase 1에서 쓰던 방식)
class DBCounterRepository:
    async def increment(self, key: str) -> int:
        # UPDATE posts SET view_count = view_count + 1 WHERE id = ?
        ...

# Redis 구현체 (Phase 5에서 새로 만드는 방식)
class RedisCounterRepository:
    async def increment(self, key: str) -> int:
        return await self.redis.incr(key)

# Service는 Protocol에 의존 — 어떤 구현체든 상관없음
class PostService:
    def __init__(self, counter_repo: CounterRepository):
        self.counter_repo = counter_repo
```

이 구조면 Service 코드 변경 없이 Repository만 교체해서 DB → Redis 전환이 됨. 테스트에서도 Mock Repository를 주입해서 Service를 단독으로 테스트할 수 있음.
...
| **typing.Protocol** | Python의 구조적 서브타이핑. 인터페이스를 명시적으로 정의해서 Repository 교체를 안전하게 만듦 | DB→Redis 카운터 전환 시 Service 코드 변경 없이 Repository만 교체 가능. 느슨한 결합의 실전 적용 |
