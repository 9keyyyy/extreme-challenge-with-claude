...
| 키워드 | 왜 알아야 하는지 |
|--------|----------------|
| **MVCC (Multi-Version Concurrency Control)** | PG가 읽기/쓰기 동시성을 처리하는 핵심 메커니즘. COUNT(*)가 느린 근본 원인 |
| **Write Amplification** | UPDATE 1건이 실제로는 old tuple 마킹 + new tuple 생성 + 인덱스 갱신. 조회수 UPDATE가 생각보다 비싼 이유 |
| **HOT Update (Heap Only Tuple)** | PG가 인덱스 갱신 없이 UPDATE하는 최적화. 조건: 인덱스된 컬럼이 안 바뀌고 같은 페이지에 공간이 있을 때 |
| **pg_stat_user_tables** | 테이블별 seq scan 횟수, 인덱스 사용 통계. 병목 진단의 첫 번째 도구 |
